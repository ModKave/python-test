# cwrtlikrwlawcyuk --> Code généré par GMAIL pour l'envoie des mails. 

# Présenter toutes les fonctionnalités du code 
# Joindre le Code Source 
# Quels modules on a utilisé / à quoi ils servent
# Mettre accent sur l'aspect cybersécurité --> Notamment avec le choix du port pour l'envoie de Mail en Tls. 
...
def envoyer_email(subject, body, to_email):
    sender_email = "apatti@guardiaschool.fr"  # Remplacez par votre adresse Gmail
    password = "cwrtlikrwlawcyuk"  # Remplacez par le mot de passe d'application généré par Google
    ...
    try:
        # Connexion au serveur Gmail via SSL
        server = smtplib.SMTP_SSL("smtp.gmail.com", 465)
        server.login(sender_email, password)  # Se connecter au serveur avec votre email et mot de passe d'application
...
def creer_compte():
    ...
    if verifier_rockyou(mot_de_passe, email):  # Passer l'email pour envoyer l'alerte
...
def verifier_rockyou(mot_de_passe, email_utilisateur):
        ...
        with open("rockyou_hashed.txt", "r") as file:
            for line in file:
                if mot_de_passe_hash == line.strip():
                    sujet = "Alerte de sécurité : Votre mot de passe est compromis"
                    corps = f"""\
Bonjour,

Nous avons détecté que le mot de passe que vous avez entré figure dans une base de données de mots de passe compromis connue (base RockYou). Cela signifie que ce mot de passe a potentiellement été exposé lors d'une fuite de données.

Par mesure de sécurité, nous vous recommandons vivement de :

1. Changer immédiatement ce mot de passe sur tous les services où il est utilisé.
2. Choisir un mot de passe unique et robuste, comportant une combinaison de lettres majuscules, lettres minuscules, chiffres et caractères spéciaux.
3. Activer l'authentification à deux facteurs (2FA) si elle est disponible sur vos comptes.
...
L'équipe Support
"""
                    envoyer_email(sujet, corps, email_utilisateur)
                    return True
        return False
    except FileNotFoundError:
...
def menu_gestion(utilisateur_id):
    while True:
        print("\n=== Menu Gestion des Produits / Compte ===")
